...
class ChatroomConsumer(WebsocketConsumer):
    ...
    def message_handler(self, event):
        ...
        context = {
            'message': message,
            'user': self.user,  # ✅ Use actual author of message
        }
...
class OnlineStatusConsumer(WebsocketConsumer):
  ...
  def online_status_handler(self, event):
    ...
    group_chats_with_user = [chat for chat in my_chats.filter(groupchat_name__isnull=False) if chat.users_online.exclude(id=self.user.id)]
        
        
    if public_chat_users or private_chats_with_user or group_chats_with_user :
       online_in_chat=True
    else:
    ...
    context = {
        ...
        'online_in_chats':online_in_chat,
